# -------------------------------------------------------
# "Differential entropy estimation with a heavy-tailed Kernel"
# "Beyond Variance: Using Differential Entropy to Detect  
...
# ========================================================
# Compute a leave-one-point-out Univariate t-Student kernel 
# density estimate
# last update: 26/May/2025
# --------------------------------------------------------

Klopout.T <- function(index, nu, h, x.var)
{
u     <- (x.var[index] - x.var[-index])/h
K.u   <- (1/(sqrt(nu)*beta(0.5,nu/2)))*(1 + (u^2)/nu)^(-(nu+1)/2)
f.hat <- mean(K.u,na.rm = TRUE)/h
return(f.hat)
}
# ========================================================
Klopout.T <- Vectorize(Klopout.T, "index")


# ========================================================
# Gets univariate differential entropy estimates based on 
# the t-Student kernel function
# last update: 26/May/2025
# --------------------------------------------------------

Entropy.T <- function(x.var, par)
{
{
index     <- 1:length(x.var)
f.hat     <- Klopout.T(index, nu = par[1], h = par[2], x.var = x.var)
log.f.hat <- log(f.hat) 
...
K.u        <- (0.5*alpha*(beta^alpha))/(beta + abs(u))^(alpha+1)
